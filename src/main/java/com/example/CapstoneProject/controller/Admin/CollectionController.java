...
public class CollectionController {
    ...
    public ResponseEntity<APIResponse> addProductToCollection(@RequestBody CollectionRequest request) {
        ...
        if (response.getStatusCode() == Code.OK.getCode()) {
            return ResponseEntity.ok(new APIResponse(Code.OK.getCode(), "Product added to collection", ""));
        }
        return ResponseEntity.ok(new APIResponse(Code.BAD_REQUEST.getCode(), "Product already exists in collection", ""));
    }
}